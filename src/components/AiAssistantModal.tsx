...
function ModelPickerRow({ provider }: { provider: typeof AI_PROVIDERS[number] }) {
  const [model, setModel] = useState(() => getActiveModel(provider.id));
  const [open, setOpen] = useState(false);

  useEffect(() => {
    setModel(getActiveModel(provider.id));
  }, [provider.id]);
